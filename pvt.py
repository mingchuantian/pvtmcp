...
from mcp import MarginCosineProduct,AddMarginProduct
__all__ = [
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        #self.mcp=MarginCosineProduct(512,num_classes)
        self.mcp=AddMarginProduct(embed_dims[3],num_classes)
        ...
        #self.head_mcp=nn.Linear(embed_dims[3],512)
    ...
    def forward(self, x, target=None,mcp=None):
        x = self.forward_features(x)
        if target is not None:
            target_long=torch.LongTensor(target.cpu())
            target_long=target_long.cuda()
        if mcp is not None:           ######
            x=self.mcp(x,target_long)
        return x
        #    x = self.head(x)
        #return x
