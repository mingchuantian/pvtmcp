...
def get_args_parser():
    ...
    parser.add_argument('--lfw_eval', action='store_true')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        model.train()   # I added to turn model.eval() back to train
        ...
        accuracy, predicts = lfw_eval.eval(model)

        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'test_{k}': v for k, v in test_stats.items()},
                     'epoch': epoch,
                     'n_parameters': n_parameters,
                     'lfw_accuracy': round(float(accuracy), 2)
                     }
